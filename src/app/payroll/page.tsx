    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }} className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p style={{ fontSize: 13, color: '#64748B', marginTop: 2 }}>Manage employees and run payroll</p>
        </div>
                  ...
                  <thead>
                    <tr style={{ borderBottom: '2px solid #E2E8F0' }}>
                      {['Employee', 'Role', 'Pay Type', 'Rate', 'Tax W/H', 'Status', ''].map((h, i) => (
                        <th key={i} style={{ padding: '12px 16px', textAlign: i > 3 ? 'center' : 'left', fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#64748B' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                            <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 99, background: emp.pay_type === 'salary' ? '#EFF6FF' : '#F0FDF4', color: emp.pay_type === 'salary' ? '#2563EB' : '#059669', textTransform: 'capitalize' }}>
                              {emp.pay_type}
                            </span>
                          </td>
                          <td style={{ padding: '14px 16px', color: '#0F172A', fontVariantNumeric: 'tabular-nums' }}>
                            <div style={{ fontWeight: 600 }}>{formatCurrency(gross)}<span style={{ fontWeight: 400, color: '#94A3B8', fontSize: 11 }}>/{payPeriod === 'weekly' ? 'wk' : payPeriod === 'biweekly' ? '2wk' : payPeriod === 'semimonthly' ? 'sm' : 'mo'}</span></div>
                            <div style={{ fontSize: 11, color: '#94A3B8' }}>
                              {emp.pay_type === 'salary' ? `${formatCurrency(rate)}/yr` : `${formatCurrency(rate)}/hr`}
                            </div>
                          </td>
                          <td style={{ padding: '14px 16px', textAlign: 'center', color: '#64748B' }}>{parseFloat(emp.tax_withholding_pct ?? 22)}%</td>
                          <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                            <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 99, background: emp.status === 'active' ? '#ECFDF5' : '#F1F5F9', color: emp.status === 'active' ? '#059669' : '#64748B', textTransform: 'capitalize' }}>
                              {emp.status}
                            </span>
                          </td>
                          <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                            <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                              <button onClick={() => openEdit(emp)} className="cursor-pointer" style={{ width: 30, height: 30, borderRadius: 6, border: '1px solid #E2E8F0', background: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748B' }}>
                                <Pencil style={{ width: 13, height: 13 }} />
                              </button>
                              <button onClick={() => setDeletingId(emp.id)} className="cursor-pointer" style={{ width: 30, height: 30, borderRadius: 6, border: '1px solid #FECACA', background: '#FEF2F2', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#EF4444' }}>
                ...
                {employees.map((emp, i) => {
                  ...
                  return (
                    <div key={emp.id} style={{ padding: '14px 16px', borderBottom: i < employees.length - 1 ? '1px solid #F1F5F9' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                      <div>
                        <p style={{ fontWeight: 500, color: '#0F172A', fontSize: 13 }}>{emp.name}</p>
                        <p style={{ fontSize: 11, color: '#94A3B8', marginTop: 2 }}>{emp.role ?? emp.pay_type}</p>
                        <p style={{ fontSize: 12, color: '#64748B', marginTop: 4 }}>{formatCurrency(gross)}/{payPeriod === 'biweekly' ? '2wk' : payPeriod.slice(0, 2)}</p>
                      </div>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 99, background: emp.status === 'active' ? '#ECFDF5' : '#F1F5F9', color: emp.status === 'active' ? '#059669' : '#64748B' }}>{emp.status}</span>
                        <button onClick={() => openEdit(emp)} className="cursor-pointer" style={{ width: 28, height: 28, borderRadius: 6, border: '1px solid #E2E8F0', background: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748B' }}>
                          <Pencil style={{ width: 12, height: 12 }} />
                        </button>
                        <button onClick={() => setDeletingId(emp.id)} className="cursor-pointer" style={{ width: 28, height: 28, borderRadius: 6, border: '1px solid #FECACA', background: '#FEF2F2', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#EF4444' }}>
            ...
            <div>
              {runs.map((run, i) => (
                <div key={run.id} style={{ borderBottom: i < runs.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                  ...
                  >
                    <div>
                      <p style={{ fontWeight: 600, color: '#0F172A', fontSize: 13 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      <div style={{ textAlign: 'right' }}>
                        <p style={{ fontSize: 13, fontWeight: 700, color: '#0F172A' }}>{formatCurrency(parseFloat(run.total_gross ?? 0))}</p>
                        <p style={{ fontSize: 11, color: '#94A3B8' }}>Gross</p>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: '#059669' }}>{formatCurrency(parseFloat(run.total_net ?? 0))}</p>
                        <p style={{ fontSize: 11, color: '#94A3B8' }}>Net</p>
                      </div>
                      <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 99, background: '#ECFDF5', color: '#059669' }}>
                        ...
                        <thead>
                          <tr style={{ borderBottom: '1px solid #E2E8F0' }}>
                            {['Employee', 'Gross', 'Taxes', 'Deductions', 'Net'].map(h => (
                              <th key={h} style={{ padding: '8px 12px', textAlign: h === 'Employee' ? 'left' : 'right', color: '#64748B', fontWeight: 600, fontSize: 11 }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {(run.entries ?? []).map((entry: any, ei: number) => (
                            <tr key={ei} style={{ borderBottom: '1px solid #F8FAFC' }}>
                              <td style={{ padding: '8px 12px', color: '#0F172A', fontWeight: 500 }}>{entry.employee_name}</td>
                              <td style={{ padding: '8px 12px', textAlign: 'right', color: '#0F172A' }}>{formatCurrency(parseFloat(entry.gross ?? 0))}</td>
                              <td style={{ padding: '8px 12px', textAlign: 'right', color: '#EF4444' }}>{formatCurrency(parseFloat(entry.taxes ?? 0))}</td>
                              <td style={{ padding: '8px 12px', textAlign: 'right', color: '#64748B' }}>{formatCurrency(parseFloat(entry.deductions ?? 0))}</td>
                              <td style={{ padding: '8px 12px', textAlign: 'right', color: '#059669', fontWeight: 600 }}>{formatCurrency(parseFloat(entry.net ?? 0))}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
      ...
      <Dialog open={showAddDialog} onClose={closeDialog}>
        ...
        <DialogContent>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <label style={{ fontSize: 13, fontWeight: 500, color: '#475569', display: 'block', marginBottom: 6 }}>Full Name</label>
            ...
            <div>
              <label style={{ fontSize: 13, fontWeight: 500, color: '#475569', display: 'block', marginBottom: 6 }}>Job Title / Role</label>
              <Input placeholder="e.g., Senior Developer" value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label style={{ fontSize: 13, fontWeight: 500, color: '#475569', display: 'block', marginBottom: 6 }}>Pay Type</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label style={{ fontSize: 13, fontWeight: 500, color: '#475569', display: 'block', marginBottom: 6 }}>Tax Withholding %</label>
      ...
      <Dialog open={showRunDialog} onClose={() => setShowRunDialog(false)}>
        ...
        <DialogContent>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label style={{ fontSize: 13, fontWeight: 500, color: '#475569', display: 'block', marginBottom: 6 }}>Period Start</label>
                <Input type="date" value={runPeriodStart} onChange={e => setRunPeriodStart(e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize: 13, fontWeight: 500, color: '#475569', display: 'block', marginBottom: 6 }}>Period End</label>
                <Input type="date" value={runPeriodEnd} onChange={e => setRunPeriodEnd(e.target.value)} />
              </div>
            </div>

            {previewEntries.length === 0 ? (
              <p style={{ fontSize: 13, color: '#64748B', textAlign: 'center', padding: '16px 0' }}>No active employees to pay.</p>
            ) : (
              <div style={{ borderRadius: 10, overflow: 'hidden', border: '1px solid #E2E8F0' }}>
                <div style={{ padding: '10px 16px', background: '#F8FAFC', borderBottom: '1px solid #E2E8F0' }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>Pay Preview</p>
                </div>
                <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid #E2E8F0' }}>
                      {['Employee', 'Gross', 'Taxes', 'Net'].map(h => (
                        <th key={h} style={{ padding: '8px 12px', textAlign: h === 'Employee' ? 'left' : 'right', color: '#64748B', fontWeight: 600 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {previewEntries.map((e, i) => (
                      <tr key={i} style={{ borderBottom: i < previewEntries.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                        <td style={{ padding: '8px 12px', color: '#0F172A' }}>{e.employee_name}</td>
                        <td style={{ padding: '8px 12px', textAlign: 'right', color: '#0F172A' }}>{formatCurrency(e.gross)}</td>
                        <td style={{ padding: '8px 12px', textAlign: 'right', color: '#EF4444' }}>{formatCurrency(e.taxes)}</td>
                        <td style={{ padding: '8px 12px', textAlign: 'right', color: '#059669', fontWeight: 600 }}>{formatCurrency(e.net)}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr style={{ borderTop: '2px solid #E2E8F0', background: '#F8FAFC' }}>
                      <td style={{ padding: '10px 12px', fontWeight: 700, color: '#0F172A', fontSize: 12 }}>Total</td>
                      <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: '#0F172A' }}>{formatCurrency(previewEntries.reduce((s, e) => s + e.gross, 0))}</td>
                      <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: '#EF4444' }}>{formatCurrency(previewEntries.reduce((s, e) => s + e.taxes, 0))}</td>
                      <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: '#059669' }}>{formatCurrency(previewEntries.reduce((s, e) => s + e.net, 0))}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
